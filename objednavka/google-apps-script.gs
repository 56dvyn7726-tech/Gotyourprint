/**
 * Objednávky triček – ukládání do Google Tabulky, potvrzovací e-maily a platba převodem.
...
function findRow(sh, id) {
  ...
  for (let i = 0; i < rows.length; i++) if (rows[i][COL.id] === id) return i + 2;
  return -1;
}

// Variabilní symbol se nikdy nepoužije dvakrát, ani po smazání objednávky.
function nextVs(sh) {
  const props = PropertiesService.getScriptProperties();
  const max = allRows(sh).reduce(function (m, r) { return Math.max(m, Number(r[COL.vs]) || 0); }, 0);
  const vs = Math.max(max + 1, Number(props.getProperty('lastVs') || 0) + 1, VS_START);
  props.setProperty('lastVs', String(vs));
  return vs;
}
...
function saveOrder(o) {
  ...
  row[COL.id] = o.id;
  row[COL.vs] = vs;
  row[COL.created] = old ? old[COL.created] : now;
  row[COL.updated] = now;
  row[COL.name] = name;
  row[COL.phone] = phone;
  row[COL.email] = email;
  ...
  return { ok: true, updated: now.toISOString(), emailSent: emailSent, payment: paymentInfo(vs, price, name) };
}
...
// force = mazání ze správy (i zaplacené), jinak zákazník může zrušit jen nezaplacenou
function removeOrder(id, force) {
  if (!validId(id)) return null;
  const sh = getSheet();
  const rowNum = findRow(sh, id);
  if (rowNum < 0) return null;
  const r = sh.getRange(rowNum, 1, 1, HEADER.length).getValues()[0];
  if (!force && r[COL.paid]) throw new Error('paid');
  sh.deleteRow(rowNum);
  return rowToOrder(r);
}
...
function setPaid(id, paid) {
  ...
  const rowNum = findRow(sh, id);
  if (rowNum < 0) return;
  const cell = sh.getRange(rowNum, COL.paid + 1);
  const wasPaid = !!cell.getValue();
  cell.setValue(paid ? new Date() : '');
  if (paid && !wasPaid) sendMail('paid', rowToOrder(sh.getRange(rowNum, 1, 1, HEADER.length).getValues()[0]));
}
...
function rowToOrder(r) {
  let items = {};
  try { items = JSON.parse(r[COL.data]); } catch (err) { /* poškozený řádek */ }
  const strip = function (v) { return String(v).replace(/^'/, ''); };
  return {
    id: r[COL.id],
    vs: r[COL.vs],
    created: iso(r[COL.created]),
    updated: iso(r[COL.updated]),
...
function listOrders() {
  return allRows(getSheet()).map(rowToOrder);
}

/* ---------- Platba: IBAN, QR Platba ---------- */
...
function czIban(account) {
  ...
  let mod = 0;
  for (let i = 0; i < digits.length; i++) mod = (mod * 10 + Number(digits[i])) % 97;
  return 'CZ' + ('0' + (98 - mod)).slice(-2) + bban;
}
...
function paymentInfo(vs, amount, name) {
  const iban = czIban(BANK_ACCOUNT);
  if (!iban) return null;
  const message = asciiUpper(PAYMENT_MESSAGE + ' ' + name).slice(0, 60).trim();
  const spd = 'SPD*1.0*ACC:' + iban + '*AM:' + Number(amount).toFixed(2) + '*CC:CZK*X-VS:' + vs + '*MSG:' + message;
  return { account: BANK_ACCOUNT, iban: iban, vs: String(vs), amount: amount, message: message, spd: spd, days: PAYMENT_DAYS };
}

// QR kód jako obrázek GIF (knihovna qrcode je na konci tohoto souboru)
function qrBlob(text) {
  const qr = qrcode(0, 'M');
  qr.addData(text);
  qr.make();
  const dataUrl = qr.createDataURL(6, 3);
  return Utilities.newBlob(Utilities.base64Decode(dataUrl.split(',')[1]), 'image/gif', 'qr-platba.gif');
}
...
// kind: 'new' | 'update' | 'cancel' | 'paid'. Vrací true, když e-mail odešel.
function sendMail(kind, o) {
  ...
  try {
    ...
    const showOrder = kind === 'new' || kind === 'update';
    const pay = showOrder ? paymentInfo(o.vs, total, o.name) : null;
    const heading = {
    ...
    const intro = {
      new: 'vaši objednávku jsme přijali. Tady je její shrnutí:',
      update: 'vaši objednávku jsme upravili. Platí tento aktuální stav:',
      cancel: 'potvrzujeme zrušení vaší objednávky. Pokud jste ji zrušili omylem, stačí ji na stránce vyplnit znovu.',
      paid: 'přijali jsme vaši platbu ' + kc(total) + ' za objednávku (variabilní symbol ' + o.vs + '). Děkujeme!',
    }[kind];

    const td = 'padding:6px 10px;border-bottom:1px solid #eee';
    const rows = keys.map(function (k) {
      const unit = PRICES[k.split(' ')[0]] || 0;
      return '<tr><td style="' + td + '">' + esc(k) + '</td>' +
        '<td style="' + td + ';text-align:right">' + o.items[k] + ' ks</td>' +
        '<td style="' + td + ';text-align:right">' + kc(unit * o.items[k]) + '</td></tr>';
    }).join('');
    ...
    if (pay) {
      const line = function (label, value) {
        return '<tr><td style="padding:3px 12px 3px 0;color:#555">' + label + '</td><td style="padding:3px 0;font-weight:bold">' + esc(value) + '</td></tr>';
      };
      let qrHtml = '';
      try {
        inlineImages.qrplatba = qrBlob(pay.spd);
        qrHtml = '<td style="vertical-align:top;padding-left:16px"><img src="cid:qrplatba" width="180" height="180" alt="QR Platba"><br>' +
          '<span style="font-size:12px;color:#777">QR Platba – naskenujte v mobilní bance</span></td>';
      } catch (err) {
        console.error('QR kód se nepodařilo vytvořit: ' + err);
      }
      payHtml = '<div style="border:2px solid #f0c02c;border-radius:10px;padding:14px 16px;margin:16px 0;max-width:520px">' +
        '<h3 style="margin:0 0 8px">Platba převodem</h3>' +
        '<table style="border-collapse:collapse"><tr><td style="vertical-align:top"><table style="border-collapse:collapse">' +
        line('Číslo účtu', pay.account) + line('Částka', kc(pay.amount)) + line('Variabilní symbol', pay.vs) + line('Zpráva', pay.message) +
        '</table></td>' + qrHtml + '</tr></table>' +
        '<p style="margin:10px 0 0;font-size:14px">Prosím zadejte <b>variabilní symbol ' + esc(pay.vs) + '</b>, podle něj platbu spárujeme s objednávkou.' +
        (pay.days ? ' Zaplaťte prosím do ' + pay.days + ' dnů.' : '') + '</p></div>';
      payText = '\nPLATBA PŘEVODEM\nČíslo účtu: ' + pay.account + '\nČástka: ' + kc(pay.amount) +
        '\nVariabilní symbol: ' + pay.vs + '\nZpráva: ' + pay.message + '\n';
    }
    ...
    const link = PAGE_URL && showOrder
      ? '<p style="margin:12px 0">Objednávku můžete do zaplacení upravit nebo zrušit na <a href="' + esc(PAGE_URL) + '">stránce objednávky</a> (otevřete ji ve stejném prohlížeči, ve kterém jste objednávali).</p>'
      : '';
    ...
    const html = '<div style="font-family:Arial,sans-serif;font-size:15px;color:#222;line-height:1.5">' +
      ...
      '<p style="margin:0 0 8px">Dobrý den, ' + esc(o.name) + ',<br>' + esc(intro) + '</p>' +
      table + payHtml + contact + info + link +
    ...
    const text = heading + '\n\n' + 'Dobrý den, ' + o.name + ',\n' + intro + '\n\n' +
      ...
      payText + (showOrder && EMAIL_INFO.length ? '\n' + EMAIL_INFO.join('\n') + '\n' : '') +
...
/* ======================================================================
   Knihovna pro QR kódy – nic zde neměňte.
   ====================================================================== */
//---------------------------------------------------------------------
//
